...
function featureUpdates_iso_id(value, filteredInputPoint, filteredInputLine, click_seq){
    ...
    filteredInputPoint = stations.filter(function(feature) {
        ...
        routeOfStation.forEach(function(route){
            ...
            rawCode = feature.properties[route].replace(/"|\[|\]/g,'') // ex. 1-20-30-1
            if (route === value) {
    ...
    //console.log(time_calc, value);
    //console.log(click_seq);
    
    // 3. Filter stations with time cost lower than value in iso bar.
    start_seq = click_seq[value]['seq'];

    let maxSeqInZoom = 1
    for (var dirSeq in time_calc[value]){
    ...
    time_cost = 0;
    input_ftr_id = [];
    //console.log('processing route: ',value);
    //console.log('max seq of route: ', maxSeqInZoom);
    while (time_cost <= parseInt(iso_slider.value) && start_seq <= maxSeqInZoom){
        //console.log(start_seq);
        if (time_calc[value][clicked_dir+'-'+start_seq]){
            time_cost += time_calc[value][clicked_dir+'-'+start_seq]['cost'];
            input_ftr_id.push(time_calc[value][clicked_dir+'-'+start_seq]['ftr_id']);
        }
        start_seq += 1;
...
let savedFeature = [];
let savedRoute = [];
function OnChangeCheckboxSave (checkbox) {
    if (exactMatch) {
...
map.on('load', function(){
    ...
    map.on('moveend', function() {
        ...
        if (features && (!radios[0].checked && !radios[1].checked && !click && !value && !iso_status)) {
            ...
            //featureUpdates(value,filtered);
            //featureUpdates_r(value,filtered_r);
        } else if (!radios[0].checked && !radios[1].checked && !click && value && !iso_status) {
